...
export default function EventInfo(props) {
    ...
    useEffect(() => {
        const getDataById = async() => {
            try {
                const results = await axios.get(`${API_PATH}/events/id/${id}`);
                const {name, createdAt, longDescription, duration} = results.data;
                await setData ({name, createdAt, longDescription, duration});
            }
            catch {
